...
main_outer_distance = 19.7; //outer side-to-side distance of insert (must fit within standard 20mm hexgrid-wall, default: 19.7
main_outer_side = (lip_outer_distance / 2) * 2 / sqrt(3); //length of connector, default: 12.99
main_inner_distance = 13.4; //inner side-to-side-distance of insert hole, default: 13.4
insert_height = 10;
lip_height = 2.5;
full_height = 46.1 / 2;
...
module _draw_insert(structure, x_pos, y_pos, tolerance = 0, decorate = true) {
  y_distance = 10 + wall_thickness; //default:11.8
  y_even = y_pos % 2;
  x_offset = y_even * (1.5 * y_distance * 2 / sqrt(3));

  x = x_pos * (3 * y_distance * 2 / sqrt(3)) + y_even * x_offset;
  y = y_pos * y_distance;
  ...
  difference() {
    ...
    for (i = [0:5]) {
      color(color_list[i]) rotate(quadrant_angles[i]) {
          ...
          rotate(30) translate(
              [
                hex_to_circular_radius(main_outer_distance) / 2 - 0.1 - tolerance,
                -.5,
                lip_height + preview_bump,
              ]
...
module _connector(structure, current, quadrant, decorate) {
  if (_check_location_populated_quadrant(structure, current, quadrant)) {
    ...
    s3 = s1 + connector_thickness + .002;
    s4 = -s2;
    s5 = s1 + connector_thickness / 2 + 0.001;
    ...
    s6 =
      ( (quadrant == 0 || quadrant == 2) && _check_location_populated_quadrant(structure, current, 1)) ? s4 + ( (connector_thickness + 0.001) * sin(60))
      : s4;
